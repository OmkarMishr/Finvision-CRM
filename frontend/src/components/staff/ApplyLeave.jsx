...
import {
  ...
  AlertCircle, ChevronLeft, ChevronRight, Plus, X,
  Send, RefreshCw, Calendar, Timer, Info, Trash2,
...
const StatusBadge = ({ status }) => {
  const cfg = STATUS[status?.toLowerCase()] || { cls: 'bg-gray-100 text-gray-500', label: status || '—', icon: Info }
  const Icon = cfg.icon
  return (
...
// ─── Leave Balance Card ───────────────────────────────────────────────────────
const BalanceCard = ({ type, used = 0 }) => {
  const cfg = LEAVE_TYPES.find(t => t.value === type) || {}
  const remaining = Math.max((cfg.max || 0) - used, 0)
  const pct       = cfg.max > 0 ? Math.min((used / cfg.max) * 100, 100) : 0
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 hover:shadow-md transition-shadow">
      <div className="flex items-start justify-between mb-3">
        ...
        <span className="text-xs text-gray-400">{used}/{cfg.max}</span>
      </div>
      <p className="text-3xl font-bold text-[#1a1a1a] tabular-nums">{remaining}</p>
...
const ApplyLeave = () => {
  ...
  const [form, setForm] = useState({
    ...
    reason:      '',
    isHalfDay:   false,
    halfDaySlot: 'morning', // morning | afternoon
    contactNo:   '',
  })
  ...
  // Balance (mock — replace with API if available)
  const [balance] = useState({
    casual: 3, sick: 2, earned: 5, halfday: 8, emergency: 1, unpaid: 0
  })
  ...
  const fetchHistory = async () => {
    ...
    try {
      const res  = await axiosInstance.get(
        API_ENDPOINTS.leave?.myHistory || '/leave/my-history'
      )
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axiosInstance.post(
        API_ENDPOINTS.leave?.apply || '/leave/apply',
        {
      ...
      setSubmitMsg({
        type: 'error',
        text: e.response?.data?.message || 'Submission failed. Please try again.',
      })
    } finally {
      setSubmitting(false)
    }
  ...
  const handleCancel = async (id) => {
    ...
    try {
      await axiosInstance.patch(
        (API_ENDPOINTS.leave?.cancel || '/leave/cancel/') + id
      )
      fetchHistory()
    } catch (e) {
      alert(e.response?.data?.message || 'Could not cancel leave.')
    }
  }
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3">
          {LEAVE_TYPES.map(t => (
            <BalanceCard key={t.value} type={t.value} used={balance[t.value] || 0} />
          ))}
        </div>
      </div>
        ...
        <div className="xl:col-span-3">
          <div className="bg-white rounded-2xl shadow border border-gray-100 overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-5 sm:p-6 space-y-5">
              ...
              {/* Half Day Toggle (auto-set for halfday type) */}
          ...
          {history.filter(h => h.status?.toLowerCase() === 'pending').length > 0 && (
            <div className="flex items-center gap-3 px-4 py-3 bg-yellow-50 border border-yellow-200 rounded-xl">
              ...
              <p className="text-sm text-yellow-800 font-medium">
                ...
                pending leave application{history.filter(h => h.status?.toLowerCase() === 'pending').length > 1 ? 's' : ''}
              </p>
            </div>
          )}
                ...
                <tbody className="divide-y divide-gray-50">
                  {pagedHist.map((h, i) => (
                    <tr key={h._id || i} className="hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-5 py-4 text-[#1a1a1a] text-xs font-medium whitespace-nowrap">
                        {fmtDate(h.fromDate)}
                      </td>
                      <td className="px-5 py-4 text-[#1a1a1a] text-xs font-medium whitespace-nowrap">
                        {fmtDate(h.toDate)}
            ...
            <div className="md:hidden divide-y divide-gray-100">
              {pagedHist.map((h, i) => (
                <div key={h._id || i} className="p-4 space-y-3">
                  ...
                  {h.reason && (
                    <p className="text-xs text-gray-500 line-clamp-2">💬 {h.reason}</p>
                  )}
