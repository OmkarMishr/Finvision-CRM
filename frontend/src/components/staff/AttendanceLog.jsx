...
const MonthCalendar = ({ records, year, month }) => {
  ...
  const firstDayOfWk = new Date(year, month, 1).getDay() // 0=Sun
...
const AttendanceLog = () => {
  ...
  const fetchLog = async (silent = false) => {
    ...
    try {
      const res  = await axiosInstance.get(API_ENDPOINTS.staffAttendance.myLog || '/staff-attendance/my-log')
      const data = Array.isArray(res.data)        ? res.data
                 : Array.isArray(res.data?.data)  ? res.data.data
                 : res.data?.records              || []
      setRecords(data)
    } catch (e) {
      console.error('Error fetching attendance log:', e)
      setRecords([])
    } finally {
      setLoading(false)
      setRefreshing(false)
  ...
  // ── Derived: calendar month records ───────────────────────────────────────
  ...
  // ── Derived: filtered table records ───────────────────────────────────────
              ...
              <div className="flex-1">
                ...
                <select
                  ...
                  className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm
                    focus:outline-none focus:ring-2 focus:ring-[#C8294A] bg-white">
                  {monthOptions.map(o => (
                        ...
                        <td className="px-5 py-4">
                          <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-xl flex flex-col items-center justify-center text-center shrink-0 ${
                              isToday ? 'bg-[#C8294A] text-white' : 'bg-gray-100 text-[#1a1a1a]'
                            }`}>
                            ...
                            <div>
                              ...
                              {isToday && (
                                <span className="text-[10px] text-[#C8294A] font-semibold">Today</span>
                              )}
                            </div>
                          </div>
                        </td>
                  ...
                  <div key={r._id || i} className={`p-4 ${isToday ? 'bg-blue-50/40' : ''}`}>
                    ...
                    <div className="grid grid-cols-3 gap-2 bg-gray-50 rounded-xl p-3">
                      <div className="text-center">
                        ...
                        {r.isLate && (
                          <p className="text-[10px] text-orange-500">+{r.lateByMinutes}m</p>
                        )}
                      </div>
                    ...
                    {r.remarks && (
                      <p className="text-xs text-gray-500 mt-2 pl-1">💬 {r.remarks}</p>
                    )}
                  </div>
                )
              })}
